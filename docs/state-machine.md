...
| `NONSTOP_GRACE` | 5분 | 연속 울음 소강 유예 — 소강 시 타이머 즉시 정지(조용한 시간 미집계), 5분 내 재개 시 멈춘 값에서 이어가고 5분 경과 시 초기화 |
...
- **연속 울음 2시간** → `emergency` 자동 전이. 어느 상태에서든 발동하되,
  실제로 우는 중일 때만 (소강 중에는 재개하는 순간 즉시 발동). 소강 시
  타이머는 즉시 정지하고(조용한 시간은 집계하지 않음), 5분 내 재개하면
  멈춘 값에서 이어가며, 5분 이상 잠잠하면 초기화된다 (`NONSTOP_GRACE`).
